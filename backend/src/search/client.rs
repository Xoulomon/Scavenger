...
impl SearchClient {
    ...
    pub fn new(config: SearchClientConfig) -> Result<Self> {
        let url = Url::parse(&config.url).context("Invalid Elasticsearch URL")?;
        ...
        // Set timeout
        transport_builder = transport_builder.timeout(std::time::Duration::from_secs(config.timeout_seconds));
    ...
    pub async fn cluster_info(&self) -> Result<serde_json::Value> {
        let response = self.client.info().send().await.context("Failed to get cluster info")?;
