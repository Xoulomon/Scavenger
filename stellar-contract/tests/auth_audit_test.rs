...
// ── #1275: end_auction — caller must be creator or admin ───────────────
...
fn test_end_auction_unauthorized() {
    ...
    env.set_auths(&[]);
    let auction_id = client.create_auction(&admin, &WasteType::Plastic, 100, 86400);
    // No auth set - anyone calling end_auction should panic
    client.end_auction(&auction_id);
}

#[test]
fn test_end_auction_creator_authorized() {
    let (_, client, _, _, _) = setup();
    env.mock_all_auths();
    let admin = Address::generate(&env);
    let auction_id = client.create_auction(&admin, &WasteType::Plastic, 100, 86400);
    // Creator can end the auction
    client.end_auction(&auction_id);
}
// Auth audit tests (issue #922)
//
